...
时，处理器会被强制切换到内核模式。第\ref{sec:cp0}章的表\ \ref{tab:cpu-mode} 列出
了这三种模式的具体设置和切换情况。
...
每个处理器模式都有自己相应的虚拟地址空间，它们是用户地址空间、管理地址空间，和内
核地址空间。每个空间都是 64 位的，并且包含一些不连续的地址空间段，最大的段为
...
XTLB 重填向量。龙 芯 GS464V 处理器核的 XTLB 重填向量与 32 位模式下 TLB 的重填向量
有相同的例外入口地址。
...
龙芯 3B1500 的地址空间按照地址的高位均匀分布到各个结点。48 位地址的高 4 位[47:44]对
