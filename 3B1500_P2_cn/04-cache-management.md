...
1. 二级混合 Cache：片上 Cache，通过 128 位 AXI 总线和处理器核通讯，共有 4 个二级
   Cache 模块。 每个二级 Cache 模块全局编址，Cache 行大小 32 字节，四个 GS464V
   处理器核共享 4M 容量，采用四路组相联的结构，采用"写回"策略。
...
14 个周期的失效代价（在 龙芯 3B1500 中处理器核和二级 Cache 需要通过交叉开关通讯，因
此还需要额外的 6 拍）。二级 Cache 以 每个时钟周期 128 位数据的速度对一级 Cache
进行回填。

一级 Cache 采用虚地址索引和物理地址标志，而二级 Cache 的索引和标志采用的都是物理
地址。 虚地址索引可能会引起地址重影（Address Aliasing）问题，目前龙芯 3B1500 设
置操作系统设置较大的页面 $\ge$ 16K）来避免这个问题，而将来会通过硬件方式解决。

龙芯 3B1500 采用了基于目录的 Cache 一致性协议来保证每个处理器核和 IO 的写能被其它处
理器核 及 IO 正确观察到。在二级 Cache 中维护了一个目录。对每个二级 Cache 中的
Cache 行，目录用 32 位 的位向量记录每个一级 Cache（包括指令和数据 Cache）是否拥
有该 Cache 行的备份。因此龙芯 3B1500  利用硬件维护了一级指令 Cache、一级数据 Cache
...
龙芯 3B1500 实现了非阻塞 Cache 技术。非阻塞 Cache 是通过允许 Cache 失效访存操作后面
...
然而在非阻塞 Cache 设计中，Cache 并不会在某个失效上暂停。龙芯 3B1500 支持多重失效下
的命中， 它最多可以支持 24 次 Cache 失效。 当一级 Cache 失效时，处理器会检查二级
Cache，看所需数据是否在其中，若二级 Cache 仍然失 效，则需要访问主存储器。

龙芯 3B1500 中的非阻塞 Cache 结构能更有效的使用循环展开和软件流水。为了尽可能最大限
度地发挥 Cache 优势，在使用访存数据的指令之前，尽可能早的执行相应的 Load 操作。

针对那些需要顺序存取的 I/O 系统，龙芯 3B1500 的默认设置是采用阻塞式的 UnCached 访问
方式。 龙芯 3B1500 提供了预取指令，可以通过 load 到 0 号定点寄存器的方式来将数据预
取到一级数据 Cache。此外龙芯 3B1500 中的 DSP 引擎可以将内存或 IO 中的数据预取到二级
Cache 中。
...
行）为 32 字节，可以存放 8 条指令。由于龙芯 3B1500 采用 128 位的读通路，所以每个时
...
龙芯 3B1500 指令 Cache 采用虚地址索引和物理地址标志的四路组相联结构。如
...
致性协议。 龙芯 3B1500 中所有片上二级 Cache 统一编址，每个二级 Cache 块都有固定的
home 结点。根据 Cache 一 致性的要求，龙芯 3B1500 的二级 Cache 具有两方面的角色：对
...
如果某个页采用非高速缓存（UnCached）算法时，那么对于在该页中任何位置的 Load 或
Store 操作，处理器都 直接发射一个双字，部分双字，字，部分字的读或写请求给主存，
而不通过任何一级 Cache。非高速缓存算法采用阻塞的方式实现。
...
由于龙芯 3B1500 中存在多个处理器核及 IO 设备可以访问主存，因此龙芯 3B1500 硬件实
现了 Cache 一 致性协议，无需通过软件干预。

### 非高速缓存加速 (一致性代码 7)


非高速缓存加速属性（UnCached Accelerated）用于优化在一 个连续的地址 空间中完成的
一系列 顺序的同一类 型的 UnCached 存数操作。该优化方法是通过设置缓冲区来收集这种
属性的存数操作。只要缓冲区不满， 就可以把这些存数操作的数据存入缓冲区中。缓冲区
和一个 Cache 行一样大小。把数据存储到缓冲区 中就和存储到 Cache 中一样。当缓冲区
满的时候，开始进行块写。在顺序存数指令的收集过程中，若 有其他类型 UnCached 存数
指令插入，则收集工作中止，缓冲区中保存的数据按字节写方式输出。 非高速缓存加速属
性可以加速顺序的 UnCached 访问，它适用于对显示设备存储的快速输出访问。

Cache 一致性
------------

龙芯 3B1500 实现了基于目录的 Cache 一致性，硬件保证了一级指令 Cache、一级数据 Cache
、二级 Cache、内存以及来自 HT 的 IO 设备之间数据的一致性，无需软件利用 Cache 指
令来强制刷 Cache。

龙芯 3B1500 中每个 Cache 行都有一个固定的宿主二级 Cache 模块。Cache 行的目录信息就
