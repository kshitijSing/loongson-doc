...
\noindent\ [HT 寄存器位域说明][] 节将给出绝大多数的 HT 配置寄存器的具体位域说明
。在这些寄存器中，
...
\begin{longtable}{|c|>{\tt}c|c|c|c|p{7.5cm}|}
  ...
  3:0   & Priority PC     & 4 & 0x1 & RW & Post 通道读写优先级。0x0：最高优先级
                                           0xF：最低优先级.  对于各个通道的优先
                                           级均采用根据时间变化提高的优先级策略
                                           ，该组存器用于 配置各个通道的初始优
                                           先级 \\ \hhline
...
龙芯 HT 控制器提供了命令、数据以及发送缓存调试寄存器，用于设置调试及观察 HT 工
作状态。

 1. 命令发送缓存大小寄存器 用于观测发送端可用各个命令通道的缓存个数。
 1. 数据发送缓存大小寄存器用于观测发送端可用各个数据通道的缓存个数。
 1. 发送缓存调试寄存器用于人工设置 HT 控制器发送端缓冲区的个数，通过增或减的方
    式对不同的发送缓存个数进行调整。
